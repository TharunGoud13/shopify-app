...
const EmptyRolesState = ({ onAction }) => (
  <EmptyState
    heading="No fresh pos list available"
    action={{
      content: "Add fresh pos list",
      onAction,
    }}
...
export default function Index() {
  ...
  return (
    <Page>
      <TitleBar title="Fresh Pos List">
        <button
          variant="primary"
          onClick={() => navigate("/app/freshposlistid/new")}
        >
          Add page
        </button>
      </TitleBar>
      <Layout>
        <Layout.Section>
          <Card padding="0">
            {data.length === 0 ? (
              <EmptyRolesState
                onAction={() => navigate("/app/freshposlistid/new")}
              />
            ) : (
              <Table data={data} />
            )}
