...
const Table = ({ data }) => (
  <IndexTable
    resourceName={{
      singular: "Fresh Notification",
      plural: "Fresh Notifications",
    }}
    ...
    <IndexTable.Row id={row.id} position={row.id}>
      <IndexTable.Cell>
        <Text variant="bodyMd" fontWeight="bold" as="span">
          <Link url={`/app/freshnotificationsid/${Id}`} rel="home">
            {row.event_type}
          </Link>
        </Text>
      </IndexTable.Cell>
      ...
      <IndexTable.Cell>
        {truncate(row.notification, { length: 50 })}
      </IndexTable.Cell>
      <IndexTable.Cell>{truncate(row.remarks, { length: 50 })}</IndexTable.Cell>
      <IndexTable.Cell>
        <Badge
